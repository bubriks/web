...
$sql = "SELECT representative.name as representative, company.name, company.regNumber, company.location, company.address, company.bankNumber, company.id
			FROM representative INNER JOIN company ON representative.companyId = company.id WHERE representative.id=$id";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);

echo "<div class='container'>
			<input type='hidden' name='companyId' class='companyId' value='".$row["id"]."'/><input type='text' name='title' class='title' placeholder='Kompānijas nosaukums' list='Companies' value='".$row["name"]."'/>
			...
			<input type='hidden' name='representativeId' class='representativeId' value='$id'/><input type='text' name='representative' class='representative' placeholder='Kompānijas pārstāvis' list='Representatives$id' value='".$row["representative"]."'/>
		</div>";

$sql = "SELECT id, name FROM representative WHERE representative.companyId = ".$row["id"]."";
